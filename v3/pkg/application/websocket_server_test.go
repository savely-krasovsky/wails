...
func TestWebSocketBroadcasterPreservesOrderPerClient(t *testing.T) {
	...
	t.Cleanup(firstClient.events.Stop)
	t.Cleanup(secondClient.events.Stop)
...
func TestWebSocketBroadcasterRemovalDiscardsPendingEvents(t *testing.T) {
	conn := new(websocket.Conn)
	firstStarted := make(chan struct{})
	releaseFirst := make(chan struct{})
	consumed := make(chan int, 2)
	window := NewBrowserWindow(1, "client")
	client := &clientInfo{window: window}
	...
	if got := <-consumed; got != 1 {
		t.Fatalf("in-flight event = %d", got)
	}
	select {
	case got := <-consumed:
		t.Fatalf("removed client consumed pending event %d", got)
	default:
	}
}
...
func TestWebSocketBroadcasterKeepsReplacementWindowOnStaleClientRemoval(t *testing.T) {
	...
	broadcaster := &WebSocketBroadcaster{
		clients: map[*websocket.Conn]*clientInfo{
			conn: {window: staleWindow, events: mailbox.New(func(*CustomEvent) {})},
		},
